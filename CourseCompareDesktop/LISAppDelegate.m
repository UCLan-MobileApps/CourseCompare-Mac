...
*/opyright (c) 2012 UCLan. All rights reserved.
//
...
- (IBAction)search:(id)sender {
    ...
    //get provider codes from the array
   
    //1 get provider name from the button menu
    NSString *b = provider.titleOfSelectedItem;
    
    //2 check name in array for code
        
    //set bools and intergers
    BOOL providerFound;
    providerFound = FALSE;    
    NSInteger idy;
    NSString *institution = [[NSString alloc] init];
    
    //set institution to code   
    for (int idx = 0; idx < [providerArray count]; idx++) {
        
        if ([b isEqualToString:[[providerArray objectAtIndex:idx] objectForKey:@"Name"]]) {
            providerFound = TRUE;
            idy = idx;
            idx = providerArray.count;
        }
    }
    if ( providerFound) {
        institution = [[providerArray objectAtIndex:idy] objectForKey:@"Code"];
    }
...
-(void)setupMenus {
    ...
    //create the headings
    NSString *ints = [NSString stringWithFormat:@"Institutions"];
    
    //load array
    providerArray = [[NSMutableArray alloc] initWithContentsOfFile:@"/tmp/providers.plist"];
    
    NSDictionary *tempDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"", @"Code", @"Any", @"Name", nil];
    NSDictionary *tempDict2 = [[NSDictionary alloc] initWithObjectsAndKeys:@"", @"Code", ints, @"Name", nil];
    
    [providerArray insertObject:tempDict atIndex:0];
    [providerArray insertObject:tempDict2 atIndex:0];
    
    NSMutableArray *tempArray = [[NSMutableArray alloc] init];
    
    for (int idx = 0; idx < [providerArray count]; idx++) {
        [tempArray addObject:[[providerArray objectAtIndex:idx] objectForKey:@"Name"]];
    }

    //remove old items and load with array items
    [provider removeAllItems];
    [provider addItemsWithTitles:tempArray];
